...
R = sqrt.(X_1 .^2 + X_2.^2)
Theta = atan.(X_2 ./ X_1)
train = cat(R, Theta, dims=3)
X_1_train_plot = reduce(vcat, X_1)
X_2_train_plot = reduce(vcat, X_2)
...
A_pure, degree_list = designMat3B2D(train, poly, max_degree, max_degree_exp, ord; body = body_order)
@show cond(A_pure)
...
R_test = sqrt.(X_1_test .^2 + X_2_test.^2)
test = cat(R_test, Theta_test, dims=3)
...
A_test, degree_list_test = designMat3B2D(test, poly, max_degree, max_degree_exp, ord; body = body_order)
yp = A_test * sol_pure
...
A_test_energy, degree_list_test  = designMat3B2D(test, poly, max_degree, max_degree_exp, ord; body = body_order)
...
# scatter!(X_1_train_plot, X_2_train_plot, plot_V2(X_1_train_plot, X_2_train_plot), seriestype=:scatter, c=0, ms=0.5, ma=0.05, label = "train")
