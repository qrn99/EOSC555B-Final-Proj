...
let NN = [5, 7, 10, 13, 17, 21, 25, 30, 35, 40], MM = NN.^2, fs=[ϕ]
    ...
    for f in fs
        for dst in distributions 
            ...
            for basis_choice in basis_choices
                ...
                plot!(P, NN, err', lw=1, m=:auto, ms=3, label="$basis_choice")
            end
            push!(plots, P)
        end
    end
    ...
    savefig(p1, exp_dir*"basic_pp_RSME_basis_inc_K_1=$K_1" * "_basis_no="*string(length(basis_choices)) * "_dst_no="*string(length(distributions)))
    p1
    end
