...
for num_sam in num_sam_list
    ...
    for i = 1:10
       ...
       cond_num_3b_pure_curr += cond(A_pure[:, length(NN2b) + 1: length(NN2b) + length(NN3b)])
    end
...
p4 = plot(num_sam_list, cond_num_3b_pure, label = "with only 3 body remove", xlabel = "num sam", ylabel = "Coniditional number")
